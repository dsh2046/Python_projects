...
Run_Info = {'run_name': 'Yukon_run1', 'product_name': 'Yukon', 'os': 'Linux', 'test_type': 0, 'build_info': 'NA',
        'start_time': date, 'end_time': date, 'total': 15, 'status': 0, 'log_file': 'NA', 'test_comment': 'NA'}
                        ...
                        for row in reader:
                            ...
                            if cur.rowcount == 0:
                                cur.execute("INSERT INTO home_functional_results(job_id, test_id, test_result, log_file, start_time, end_time)VALUES(%s, %s, %s, %s, %s, %s)",
                                            (str(run_id), str(testcase_id), str(row['Result']), str(x['log_file']), str(x['start_time']), str(x['end_time'])))
        if csv_amount == 0:
            print 'No csv found!'
...
if __name__ == '__main__':
    resultsUpload('/home/samuel.deng/Python/db_config.ini').upload(Run_Info, '/home/samuel.deng/Python/CSV')
