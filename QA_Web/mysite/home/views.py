...
class IndexView(View):
    def get(self, request):
        ...
        if username == 'anybody':
            ...
            result = product.objects.all().using(username)
            try:
                product_name = request.GET.get('pn', '')
                testtype_name = request.GET.get('ttn', '')
                if product_name == '':
                    return render(request, 'home/pages/index.html', {'side': result, 'result': result, 'username': username, 'product_name': product_name})
                else:
                    if testtype_name == '':
                        ...
                        if testtype_name == 'Functional':
                            result = product.objects.using(username).get(name=product_name)
                            result2 = product.objects.all().using(username)
                            detail_table = functional_test_run.objects.using(username).filter(product_id=int(result.id))
                            ...
                            result = product.objects.using(username).get(name=product_name)
                            result2 = product.objects.all().using(username)
                            detail_table = performance_test_run.objects.using(username).filter(product_id=int(result.id))
                            return render(request, 'home/pages/index.html', {'side': result2, 'detail': detail_table, 'username': username, 'product_name': product_name, 'test_type': testtype_name})
            except:
...
class CompareView(View):
    def get(self, request):
        ...
        if test_type == 'Functional':
            product_id = functional_test_run.objects.using(username).get(id=int(run_id)).product_id
            test_run_time = functional_test_run.objects.using(username).get(id=int(run_id)).start_time
            test_run = functional_test_run.objects.using(username).filter(product_id=int(product_id)).order_by('start_time')
            try:
                lastrun_id = functional_test_run.objects.using(username).filter(start_time__lt=test_run_time).order_by('-start_time')[:1].values_list('id')[0]
                run_result = functional_results.objects.using(username).filter(job_id=int(run_id)).values_list()
                lastrun_result = functional_results.objects.using(username).filter(job_id=lastrun_id).values_list()
                for run in run_result:
                    testcase_name = functional_testcase.objects.using(username).get(id=run[2]).test_name
                ...
                for k, v in thisrun_testcaseDic.items():
                    if k in lastrun_testcaseDic:
                        if v == u'Pass' and lastrun_testcaseDic[k] == u'Fail':
                            progress.append(k)
                        elif v == u'Fail' and lastrun_testcaseDic[k] == u'Pass':
                            regress.append(k)
                    else:
            ...
            product_id = performance_test_run.objects.using(username).get(id=int(run_id)).product_id
            test_run_time = performance_test_run.objects.using(username).get(id=int(run_id)).start_time
            test_run = performance_test_run.objects.using(username).filter(product_id=int(product_id)).order_by('start_time')
            try:
                lastrun_id = performance_test_run.objects.using(username).filter(start_time__lt=test_run_time).order_by('-start_time')[:1].values_list('id')[0]
                run_result = performance_results.objects.using(username).filter(job_id=int(run_id)).values_list()
                lastrun_result = performance_results.objects.using(username).filter(job_id=lastrun_id).values_list()
                for run in run_result:
                    testcase_name = performance_testcase.objects.using(username).get(id=run[2]).test_name
                    thisrun_testcaseDic[testcase_name] = run[3]
                for run in lastrun_result:
                    testcase_name = performance_testcase.objects.using(username).get(id=run[2]).test_name
                    lastrun_testcaseDic[testcase_name] = run[3]
...
def table(request):
    ...
    if username == 'anybody':
        ...
        # result = cur.fetchall()
        try:
            result = functional_test_run.objects.all().using(username)
...
def detail(request):
    if 'username' in request.session:
        ...
        test_type = request.GET.get('test_type', '')
        if test_type == 'Functional':
            cur.execute('select home_functional_testcase.test_name, home_functional_testcase.test_type, home_functional_results.start_time, home_functional_results.end_time, home_functional_results.test_result, home_functional_results.log_file, home_functional_test_run.total_ran from home_functional_results, home_functional_testcase, home_functional_test_run where home_functional_results.test_id=home_functional_testcase.id and home_functional_results.job_id='+job_id+' and home_functional_test_run.id=home_functional_results.job_id')
            result = cur.fetchall()
